...
def sample(logits, temperature: float=1.0, top_k: int=0, top_p: float=1.0, sample_logits=True):
    # upcast for stable softmax/multinomial in bf16/fp16
    logits = logits[:, -1, :].float() / max(temperature, 1e-5)
    if top_k > 0 or top_p < 1.0:
        logits = top_k_top_p_filtering(logits, top_k=top_k, top_p=top_p)
    probs = F.softmax(logits, dim=-1)

    # guard against degenerate rows (prevents device-side assert)
    bad = (~torch.isfinite(probs)).any(dim=-1) | (probs.sum(dim=-1) <= 0)
    if bad.any():
        idx = torch.argmax(logits, dim=-1, keepdim=True)            # greedy for bad rows
        ok = ~bad
        if ok.any() and sample_logits:
            idx_ok = torch.multinomial(probs[ok], num_samples=1)
            idx[ok] = idx_ok
        return idx, probs
...
@torch.no_grad()
def generate(model, cond, max_new_tokens, cfg_scale=1.0, cfg_interval=-1, **sampling_kwargs):
    if model.model_type == 'c2i':
        T = model.cls_token_num
        if cfg_scale > 1.0:
            cond_null = torch.full_like(cond, fill_value=model.num_classes)  # special "null" class id
    ...
    seq = torch.empty((max_batch_size, T_new), dtype=torch.long, device=device)
